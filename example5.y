...
program ::= NAME(A) EQ expr(B).       {    
    if (A.symt->funcptr || A.symt->funcptr2) {
        printf("%w is a function! Must type in  rightside!\n", A.symt->name);
        A.symt->value = 0.0;
    } else {
...
expr(A) ::= NAME(B).                { 
    if (B.symt->funcptr || B.symt->funcptr2) {
        printf("%w is a function! Must type like %w(number)!", B.symt->name, B.symt->name);
        A.value = 0.0;
    } else {
...
expr(A) ::= NAME(B) LP expr(C) COMMA expr(D) RP.  {
    if (B.symt->funcptr2) {
        ...
        printf("%w function undifined! ", B.symt->name);
    }
    A.value = B.symt->value;
}
...
%code {
    ...
    static void addfunc(char* name, double (*func)(double x), struct Symbol* symtab) {
        struct Symbol* sp = symlook(name, symtab);
        sp->funcptr = func;
    }

    static void addfunc2(char* name, double (*func)(double x, double y), struct Symbol* symtab) {
        struct Symbol* sp = symlook(name, symtab);
        sp->funcptr = func;
    }
